...
graph TD
    ...
    subgraph LAN ["🏠 Локальная сеть (Wi-Fi)"]
        B -->|Сырой PCM| C[🖥️ FastAPI Сервер на ПК]
        C -->|Запрос остатков| D[(🗄️ SQLite business.db)]
        D -->|RAG-контекст| C
        C -->|URL аудио| B
    end

    subgraph CLOUD ["☁️ Облачные AI-сервисы (интернет)"]
        E[🎙️ OpenAI Whisper STT]
        F[🧠 GPT-4o-mini]
        G[🔊 Microsoft Edge TTS]
    end

    C -->|WAV файл| E
    E -->|Распознанный текст| C
    C -->|Промпт + контекст склада| F
    F -->|Текстовый ответ| C
    C -->|Текст для озвучки| G
    G -->|MP3 файл| C
    ...
    style CLOUD fill:#fff8e1,stroke:#f9a825,stroke-width:2px
    style LAN fill:#e8f5e9,stroke:#388e3c,stroke-width:2px
```
